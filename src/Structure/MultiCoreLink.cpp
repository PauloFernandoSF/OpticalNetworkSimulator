...
MultiCoreLink::MultiCoreLink(Topology* topPointer, unsigned int origimNode, 
    ...
    this->coreVector.resize(this->GetTopology()->GetNumCores());
...
void MultiCoreLink::Initialize(){
    ...
    for(unsigned int c = 0;c < this->GetTopology()->GetNumCores();c++){
        this->coreVector.at(c) = (std::make_shared<Core>(c,this->GetTopology()->GetNumSlots()));   
    }
}
